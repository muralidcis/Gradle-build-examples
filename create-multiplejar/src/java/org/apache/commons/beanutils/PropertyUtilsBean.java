...
public class PropertyUtilsBean {
    ...
    public Object getIndexedProperty(Object bean,
	  ...
	  throw new NoSuchMethodException("Unknown property '" +
        ...
        if (!value.getClass().isArray()) {
	      ...
	      //
	      // Extended by Hari to create objects if the list has a generic type associated with it
	      //
		...
		for (int counter = ((List)value).size(); counter <= index; counter++) {
		  ((List)value).add(c.newInstance());
		}
	      } catch (Exception e) {
		e.printStackTrace();
	      }
	...
	//
	// Hari: if the value of the target type is an Enum, call the
	// valueOf() method of the corresponding Enum first before
	// setting the property on the bean
	//
	...
	if (descriptor.getPropertyType().isEnum()) {
	  try {
	    ...
	    //
	    // Get its valueOf() method
	    //

	    Method m = clazz.getMethod("valueOf", java.lang.Class.class, java.lang.String.class);
	    
	    //
	    // ... and invoke it
	    //

	    values[0] = m.invoke(null, new Object[] {clazz, value});
	  } catch (Exception e) {
